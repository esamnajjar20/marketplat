 ...
 * Mirrors CreateServiceCategoryButton.tsx exactly (same slugify
 * helper, same field layout including the optional icon field —
 * ProductCategory has one, same as ServiceCategory).
 */
...
/** Same as CreateServiceCategoryButton's slugify — backend's
 * createProductCategorySchema requires the identical /^[a-z0-9-]+$/ shape. */
  ...
  return (
    <>
      <Button size="sm" className="gap-1.5" onClick={() => setOpen(true)}>
        <Plus className="h-4 w-4" /> فئة منتج جديدة
      </Button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>إنشاء فئة منتج جديدة</DialogTitle></DialogHeader>
